    ...
    <div class="row">
        ...
        <div class="col-md-4">
            ...
            <div class="fw-semibold
                {{ $pengajuan->status == 'aktif' ? 'text-success' :
                   ($pengajuan->status == 'ditolak' ? 'text-danger' : 'text-warning') }}">
...
<div class="mt-4 d-flex justify-content-end gap-2">
    ...
    @if ($pengajuan->status == 'menunggu')
        ...
        <form id="approveForm"
            action="{{ route('pemilik.pengajuan.approve', $pengajuan->id) }}"
            method="POST">
            @csrf
        </form>

        <form id="rejectForm"
            action="{{ route('pemilik.pengajuan.reject', $pengajuan->id) }}"
            method="POST">
            @csrf
